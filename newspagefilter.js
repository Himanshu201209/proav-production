...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function filterItems() {
    ...
    items.forEach((item) => {
      ...
      item.classList.remove("visible1", "visible2", "visible3"); // Remove visibility classes from all items
    });
  ...
  function clearFilter() {
    ...
    items.forEach((item) => {
      ...
      item.classList.remove("visible1", "visible2", "visible3"); // Ensure visibility classes are removed
    });
  ...
  function showPage(page) {
    ...
    filteredItems.forEach((item, index) => {
      if (index >= start && index < end) {
        ...
        // Add the appropriate visible class based on visibleCount
        if (visibleCount < 3) {
          item.classList.add(`visible${visibleCount + 1}`);
          visibleCount++;
        } else {
          // Remove any previously added visible classes if more than 3
          item.classList.remove("visible1", "visible2", "visible3");
        }
      } else {
        item.classList.add("hidden");
        // Ensure all visible classes are removed if hidden
        item.classList.remove("visible1", "visible2", "visible3");
      }
    });
  }
